...
class TestPostgresLoaderIntegration:
    ...
    def test_load_recent_events(self):
        """Test loading recent events from database."""
        from LedgerFlow.data.loader import load_recent_events
        
        # Load small sample
        df = load_recent_events(days=1, chunk_size=100)
        
        # Verify structure
        expected_columns = ["event_id", "user_id", "event_type", "event_timestamp", "amount", "session_id"]
        ...
        # If we got data, verify it's within date range
        if not df.empty:
            end_date = datetime.now()
            start_date = end_date - timedelta(days=1)
            
            assert df["event_timestamp"].min() >= start_date
            assert df["event_timestamp"].max() <= end_date

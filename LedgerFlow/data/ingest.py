...
from .loader import load_recent_events
...
def ingest_raw_events(
    ...
    try:
        ...
        if len(validated_df) <= 10000:
            single_file_path = output_path / "events_latest.parquet"
            validated_df.to_parquet(
                single_file_path,
                engine="pyarrow",
                compression="snappy",
        ...
        logger.info(
            f"  - Date range: {validated_df['event_timestamp'].min()} "
            f"to {validated_df['event_timestamp'].max()}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days of history to load"
    )
    parser.add_argument(
        "--max-users", type=int, help="Maximum number of unique users to load"
    )
    parser.add_argument(
        "--output-dir", default="data/raw", help="Output directory for Parquet files"
    )
    parser.add_argument(
        "--skip-report",
        action="store_true",

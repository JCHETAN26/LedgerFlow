...
RawEventSchema = DataFrameSchema({
    "event_id": Column(
        ...
        checks=[
            ...
            Check.str_matches(r"^[a-zA-Z0-9_-]+$", error="event_id contains invalid characters"),
        ]
    ),
    "user_id": Column(
        str,
        ...
        checks=[
            ...
            Check.str_matches(r"^[a-zA-Z0-9_-]+$", error="user_id contains invalid characters"),
        ]
    ),
    "event_type": Column(
        str,
    ...
    "amount": Column(
        ...
        checks=[
            ...
            Check.less_than_or_equal_to(1_000_000, ignore_na=True),  # Reasonable upper bound
        ]
    ),
    "session_id": Column(
        str,
...
def validate_raw_events(
    ...
    Args:
        ...
        fail_fast: If True, raise on first validation error. If False, collect all errors.
...
def _validate_business_rules(df: pd.DataFrame) -> None:
    ...
    if not purchase_without_amount.empty:
        error_msg = f"Found {len(purchase_without_amount)} purchase events without amount"
    ...
    if not non_purchase_with_amount.empty:
        error_msg = f"Found {len(non_purchase_with_amount)} non-purchase events with amount"
        logger.error(error_msg)
        raise ValueError(error_msg)
    ...
    if len(df) > 0:
        ...
        if time_span.days > 365 * 5:  # 5 years
            logger.warning(f"Data spans {time_span.days} days - consider filtering to smaller time range")
    
    logger.debug("Business rule validation passed")


def generate_validation_report(df: pd.DataFrame, schema: DataFrameSchema = RawEventSchema) -> dict:
    ...
    if not df.empty:
        report["statistics"] = {
            "date_range": {
                "min": df["event_timestamp"].min().isoformat() if pd.notna(df["event_timestamp"].min()) else None,
                "max": df["event_timestamp"].max().isoformat() if pd.notna(df["event_timestamp"].max()) else None,
            },
            "user_count": df["user_id"].nunique(),
            "event_type_distribution": df["event_type"].value_counts().to_dict(),
            "null_counts": df.isnull().sum().to_dict(),
            "purchase_stats": {
                "count": len(df[df["event_type"] == "purchase"]),
                "total_amount": df[df["event_type"] == "purchase"]["amount"].sum() if "amount" in df.columns else None,
                "avg_amount": df[df["event_type"] == "purchase"]["amount"].mean() if "amount" in df.columns else None,

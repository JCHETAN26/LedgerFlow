...
def load_events_from_postgres(
    ...
    try:
        # Load into a DataFrame, streaming in chunks for large result sets.
        with engine.connect() as connection:
            if chunk_size:
                chunks = list(
                    pd.read_sql(query, connection, params=params, chunksize=chunk_size)
                )
                df = (
...
if __name__ == "__main__":
    ...
    _required = ["LEDGERFLOW_DB_NAME", "LEDGERFLOW_DB_USER", "LEDGERFLOW_DB_PASSWORD"]
    if not all(os.getenv(var) for var in _required):
    ...
    try:
        ...
        if not df.empty:
            ...
            print(
                f"   Date range: {df['event_timestamp'].min()} "
                f"to {df['event_timestamp'].max()}"
            )

...
class BaseFeature(ABC):
    ...
    def validate_output(self, result: pd.Series) -> None:
        ...
        """
        assert result.name == self.name, f"Series name {result.name} != expected {self.name}"
        ...
        if self.output_dtype == "int":
            assert pd.api.types.is_integer_dtype(result), f"{self.name} should be integer dtype"
        elif self.output_dtype == "float":
            assert pd.api.types.is_float_dtype(result), f"{self.name} should be float dtype"
        elif self.output_dtype == "bool":
            assert pd.api.types.is_bool_dtype(result), f"{self.name} should be boolean dtype"